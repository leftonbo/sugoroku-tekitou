...
- **`src/utils/math-utils.ts`**: 数学関数、確率計算、累積コスト計算
- **`src/utils/xorshift-random.ts`**: XorShift128高品質疑似乱数生成器

### 重要なゲームシステム

**ダイスシステム（src/systems/dice-system.ts）**:
- **手動ダイス**: 複数個対応、手動実行
- **自動ダイス**: 6種類（4/6/8/10/12/20面）、段階的解禁
- **タイマー管理**: 各ダイスの実行間隔制御、負荷システム対応
- **アップグレード**: 速度（レベル1-20で間隔短縮）・個数（アセンションで増加）
- **累積コストシステム**: 全アセンション累積投資を考慮した価格計算

**盤面システム（src/systems/board-system.ts）**:
- **動的生成**: レベルごとの盤面再生成、XorShift128による真のランダム
- **マス目種類**: 空白、クレジット、進む、戻るマス
- **特殊ボーナスマス**: 1%確率でボーナスクレジットマス（5倍獲得）、状態管理機能
- **特殊配置**: 固定戻るマス（レベル10以降、90-99マス目）
- **効果適用**: マス目到達時の効果処理
- **状態保存**: ボーナスマス使用後の状態を差分形式で効率保存

**アップグレードシステム（src/systems/upgrade-system.ts）**:
- **段階的価格**: 累乗による価格上昇（1.5倍、2倍等）
- **解禁システム**: ダイス種類の段階的解禁
- **累積コスト計算**: 全アセンション累積レベルを考慮した公正な価格計算
...
**XorShift128乱数生成システム（src/utils/xorshift-random.ts）**:
- **高品質疑似乱数**: Linear Congruential Generatorの問題を解決
- **独立した乱数ストリーム**: 盤面生成、マス効果、ボーナス確率で異なるシード
- **再現可能性**: 同じシードで同じ盤面を生成
- **長周期**: 2^128-1の周期を持つ高品質な乱数生成
...
- **統計**: `stats` - プレイ統計、実績記録
- **ボードの状態管理**: `boardStates` - レベル・位置別のマス状態差分
- **プレステージアップグレード**: `prestigeUpgrades` - 永続強化レベル
...
- **乱数生成**: 新しいランダム要素はXorShift128を使用し、独立したシードを設定
- **状態保存**: 新しい状態は差分保存の考慮、メモリ効率を優先

### パフォーマンス考慮
- **DOM操作最小化**: HTML再生成を避け、既存要素の更新を優先
- **メモリ管理**: イベントリスナーのクリーンアップ、タイマー管理
- **バッチ処理**: 複数の状態変更をまとめて処理
- **効率的データ構造**: BoardStateDiffによる差分保存でメモリ使用量削減
- **乱数生成最適化**: XorShift128の計算コストと品質のバランス
